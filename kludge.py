...
import kludgecollection as collect
...
class KludgeCollector:
    ...
    def printit(self):
        (opts, args) = getopt.getopt(sys.argv[1:], 'milqfeavrho:e:', [
            'help',
            'memory',
            'inline',
            'logs',
        ...
        print 'Running Kludge with ' + str(sys.argv) + ' on ' \
...
#           print var.level
    ...
    def manpage(self):
        ...
        print 'Usage: kludge.py [Options]'
        print '\t  -m/--memory: Collect Memory'
        print '\t         This option will perform a winpmem dump of memory'
        print '\t  -i/--inline: Perform inline Memory Data Collection'
    ...
    def main(self, argv):
        sys.stderr.write('Kludge {0}'.format(constants.VERSION)
                         + '\t(https://github.com/Kvetch/)\n')
        sys.stderr.flush()
        try:
            (opts, args) = getopt.getopt(argv, 'milqfeavrho:e:', [
                'help',
                'memory',
                'inline',
                'logs',
        ...
        for (opt, arg) in opts:
            ...
            # Establish the collection level

            var.level = var.quick + var.full


            # This will offer us 3 collection levels
            # 1. Quick
            # 2. Full
            # 3. Quick then Full (a feature people are looking for)

if __name__ == '__main__':
    StartUp = KludgeCollector()
    StartUp.printit()
    StartUp.running()
